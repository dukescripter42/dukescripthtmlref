 ...
 */
package org.dscript.hmtlinputs.dukescripthtmlref.js;
...
public class TemplateRegistration {
    ...
    public static Closeable register(String id, String template) throws IllegalStateException {
        ...
        if (obj == null) {
            throw new IllegalStateException("Cannot register a template with id " + id + " and template " + template + " (trying to register twice?)");
        }
    ...
    @JavaScriptBody(args = {"id", "template"}, body = "var my_template = document.getElementById(id);\n"
            ...
            + "xhr.send();"
            + "var newContent = document.createTextNode(xhr.responseText); \n"
